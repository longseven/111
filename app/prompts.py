...
- difficulty：'易'/'中'/'难'。
- adaptation_reason：用 1-2 句**具体、真实**地说明本新题相对原题改了哪些地方（数值、情境、问法或题型的具体变化），以及如何对应用户所给的改编条件。必须基于实际发生的改动来写，**严禁空泛套话**（如"使题目更灵活""更贴近生活""提升思维能力"），也不得声称未发生的改动。示例："把原题'骑车行程'情境改为'购物找零'，单价与数量换成新数值；题型仍为应用题，按要求把难度从易升到中（多加一步减法）。"
